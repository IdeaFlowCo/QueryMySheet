...
import { parseCSV, fetchGoogleSheet, parseUploadedFile } from '../server/sheets';
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      try {
        ...
        return res.status(400).json({
          error: `Unable to parse uploaded file: ${error instanceof Error ? error.message : 'Unknown error'}`
        });
      }
    } 
    // Process Google Sheet URL if provided
      ...
      try {
        ...
        return res.status(400).json({
          error: `Unable to fetch or parse Google Sheet: ${error instanceof Error ? error.message : 'Unknown error'}`
        });
      }
    } 
    else {
      return res.status(400).json({ error: 'Either a file or sheetUrl is required' });
    }
